...
void main()
{
    ...
    for (int i = 0; i < ssbo_lights.length(); ++i)
    {
        ...
        {
            case DIRECTIONAL_LIGHT_SOURCE: light_sum += CalculateDirectionalLight(ssbo_lights[i]);  continue;
            case POINT_LIGHT_SOURCE: light_sum += CalculatePointLight(ssbo_lights[i]);              continue;
            case SPOT_LIGHT_SOURCE: light_sum += CalculateSpotLight(ssbo_lights[i]);                continue;
        }  
    }
