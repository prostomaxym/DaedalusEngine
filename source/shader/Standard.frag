...
float BilinearInterpolation(sampler2D shadow_map, vec2 tex_coords) 
{
    ivec2 texel_coord = ivec2(tex_coords * textureSize(shadow_map, 0));
    vec2 texel_offset = fract(tex_coords * textureSize(shadow_map, 0));

    float top_left = texture(shadow_map, (texel_coord + vec2(0.5, 0.5)) / textureSize(shadow_map, 0)).r;
    float top_right = texture(shadow_map, (texel_coord + vec2(1.5, 0.5)) / textureSize(shadow_map, 0)).r;
    float bottom_left = texture(shadow_map, (texel_coord + vec2(0.5, 1.5)) / textureSize(shadow_map, 0)).r;
    float bottom_right = texture(shadow_map, (texel_coord + vec2(1.5, 1.5)) / textureSize(shadow_map, 0)).r;
...
float CalculateShadow(Light p_light) 
{
...
vec3 BlinnPhong(Light p_light, vec3 p_light_dir, float p_luminosity)
{
    ...
    const float shadow = p_light.cast_shadows ? CalculateShadow(p_light) : 1.0;  
