...
void OpenGLShaderLibrary::RecompileAllShaders(const std::set<std::string>& shader_names, const std::string& path)
{
	for (auto& shader_name : shader_names)
	{
		...
		shader = std::make_shared<OpenGLShader>(path + shader_name + ".vert", path + shader_name + ".frag");
		shader->SaveBinary(path + "cache/" + shader_name + ".dlshader");
		shader->SetName(shader_name);
		m_shaders.insert({ shader_name, shader });
	}
}

void OpenGLShaderLibrary::LoadCachedShaders(const std::vector<std::string>& cached_shader_names, const std::string& path)
{
	for (auto& shader_name : cached_shader_names)
	{
		...
		try
		{
			shader = std::make_shared<OpenGLShader>(path + "cache/" + shader_name + ".dlshader");
		}
...
void OpenGLShaderLibrary::LoadNotCachedShaders(const std::set<std::string>& all_shader_names, const std::vector<std::string>& cached_shader_names, const std::string& path)
{
...
std::set<std::string> OpenGLShaderLibrary::QueryFileNames(const std::string& path, bool ignore_extention) const
{
